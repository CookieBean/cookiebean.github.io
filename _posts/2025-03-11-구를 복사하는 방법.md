...
**Proof.** 먼저 첫 번째 결과는 수학적 귀납법을 사용하여 증명한다. 참고로, 본 증명에서는 행렬을 이용해서 회전변환 좌표들을 계산할텐데 $a$와 $b$에 해당하는 행렬은 아래와 같다.
